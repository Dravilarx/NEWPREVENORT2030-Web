    ...
    async function handleAdmision(e: React.FormEvent) {
        ...
        try {
            ...
            if (bateriaSugerida && bateriaSugerida.bateria_items) {
                const itemsToInsert = bateriaSugerida.bateria_items.map((bi: BateriaItem) => ({
                    atencion_id: newAtencion.id,
                    item_nombre: bi.prestaciones?.nombre || 'Examen',
                    valor_encontrado: null,
                    es_alerta: false,
                    responsable_rol: bi.prestaciones?.categoria || 'General'
                }));
                ...
                if (itemsToInsert.length > 0) {
                    ...
                    if (iError) console.error('Error insertando ítems de batería:', iError);
                }
            }
